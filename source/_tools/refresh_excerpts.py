#!/usr/bin/env python3
"""Vendor code excerpts from the two anchor repos into ``source/examples/``.

Driven by the manifest ``source/_tools/excerpts.yaml``. Each entry names a
...
4.  Regenerates ``source/examples/SOURCE.md`` with one row per captured
    excerpt.
...
Usage:
    python3 source/_tools/refresh_excerpts.py              # refresh all
    python3 source/_tools/refresh_excerpts.py --check      # verify only
...
# the author's machine. Both reference impls now live as sibling checkouts of
# this book repo, so paths are computed lazily and CI can override either via
# LAZY_KB_WIKI_PATH / LAZY_AI_CODER_PATH.
KNOWN_REPOS = {
    "lazy-kb-wiki": Path(
        os.environ.get("LAZY_KB_WIKI_PATH", REPO_ROOT.parent / "lazy-kb-wiki")
    ),
    "lazy-ai-coder": Path(
...
DISPLAY_REPO_NAMES = {
    "lazy-kb-wiki":  "prose-layer-reference-impl",
