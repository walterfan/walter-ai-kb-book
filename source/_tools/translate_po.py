...
.po file under source/locale/zh_CN/LC_MESSAGES/, writes translations into the
active (non-obsolete) entries, and reports anything missing or extra.

Usage
-----

    poetry run python source/_tools/translate_po.py \\
        source/_tools/translations/ch02-ir-rag-primer.zh.py

The translations module must define:

    TRANSLATIONS: dict[str, str]  # msgid -> msgstr (Chinese)
    PO_PATH:      str             # path relative to source/locale/zh_CN/LC_MESSAGES/
